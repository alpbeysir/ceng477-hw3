...
void draw_model(const ModelData& model, Shader& s, const glm::vec3& position, const float y_rot_angle, const glm::vec3& scale, const glm::vec3& color)
{
    ...
    // prerot = glm::rotate(prerot, -glm::radians(y_rot_angle), { 0,1,0 });
    ...
    // std::cout << trans << prerot << "\n";
    if(y_rot_angle != angl) {
        // std::cout << position << "\n";
        // std::cout << (prerot * glm::vec4(position + glm::vec3{0, 0, 1}, 1.0)) << "\n";
        // std::cout << (trans * glm::vec4(position + glm::vec3{0, 0, 1}, 1.0)) << "\n\n";
        angl = y_rot_angle;
    }
    s.SetUniformMat4f("u_Prerot", prerot);
