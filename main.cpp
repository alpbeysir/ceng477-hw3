...
float bg_vertices[20] = {
    /*Position data*/ -1.0f, -1.0f, /*Color data*/ 0.8f, 0.0f, 0.0f,
    /*Position data*/ 1.0f, -1.0f, /*Color data*/ 0.0f, 0.8f, 0.0f,
    /*Position data*/ 1.0f, 1.0f, /*Color data*/ 0.0f, 0.0f, 0.8f,
    /*Position data*/ -1.0f, 1.0f, /*Color data*/ 0.8f, 0.0f, 0.0f,
};
unsigned int bg_indices[6] = {
    0, 1, 2,
    2, 3, 0
};
...
int main(int argc, char* argv[])
{
    ...
    FontData font = load_font("./Titillium-Bold.otf");
    Shader text_shader("example_text.shader");

    Shader background_shader("background.shader");
    ...
    GLuint bgvao, bgidx;
    glGenBuffers(1, &bgvao);
    glGenBuffers(1, &bgidx);
    glBindBuffer(GL_ARRAY_BUFFER, bgvao);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, bgidx);
    glBufferData(GL_ARRAY_BUFFER, 20 * sizeof(float), bg_vertices, GL_STATIC_DRAW);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * sizeof(unsigned int), bg_indices, GL_STATIC_DRAW);
    glEnableVertexAttribArray(4);
    glEnableVertexAttribArray(5);
    glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), 0);
    glVertexAttribPointer(5, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (char*)(2 * sizeof(float)));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float next = draw_text(font, text_shader, text, glm::vec2(-10, -9.9), text_size, glm::vec3(1,1,0));
        if(is_animating) {
            draw_text(font, text_shader, " WAIT", glm::vec2(-10+next, -9.9), text_size, glm::vec3(1,0,0));
        } else {
            draw_text(font, text_shader, " MOVE", glm::vec2(-10+next, -9.9), text_size, glm::vec3(0,1,0));
        }
        delete[] text;
